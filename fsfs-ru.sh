...
Использование: $0 [ФЛАГИ] [ ПУТЬ_К_МЕДИАТЕКЕ ]

ФЛАГИ:
  -h, --help            Показать справку.
  -d, --dry-run         Режим проверки без изменений.
  -s, --skip-existing   Пропускать альбомы, если папка splitted уже содержит .flac файлы.
  -k, --delete-original Удалить исходный аудиофайл и .cue после успешной нарезки.
  -i, --install-deps    Установить зависимости.
  -r, --remove-deps     Удалить зависимости.
EOF
    exit 0
}
...
check_root() {
    if [ "$EUID" -ne 0 ]; then
        if command -v sudo &>/dev/null; then SUDO="sudo"; else echo "[!] Ошибка: Требуются права root или sudo."; exit 1; fi
    else SUDO=""; fi
}
...
detect_pm() {
    ...
    else echo "[!] Ошибка: Не удалось определить пакетный менеджер вашей ОС."; exit 1; fi
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -*) echo "[!] Ошибка: Неизвестный флаг '$1'"; exit 1 ;;
        *) TARGET_DIR="$1"; shift ;;
    esac
done
...
if [ "$DRY_RUN" -eq 0 ]; then
    ...
    if [ ${#missing_cmds[@]} -gt 0 ]; then
        echo "[!] Отсутствуют утилиты: ${missing_cmds[*]}"
        echo "Запустите: $0 --install-deps"
        exit 1
    fi
fi
...
if [ -z "$audio_file" ]; then
    ...
    if [ "$SKIP_EXISTING" -eq 1 ] && [ -d "$dir/$OUT_DIR_NAME" ] && find "$dir/$OUT_DIR_NAME" -maxdepth 1 -iname "*.flac" -print -quit | grep -q .; then
        echo "  [↪] Папка '$OUT_DIR_NAME' уже содержит файлы. Пропускаем."
        continue
    fi
    ...
    (
        ...
        if [ -n "$bits" ] && [ "$bits" -gt 16 ] 2>/dev/null; then
            echo "-> Обнаружен Hi-Res FLAC (${bits}-bit). Нарезка через ffmpeg..."
            ...
            for bp in "${breakpoints[@]}"; do
                ...
                if [ "$bp" == "end" ]; then
                    echo "    [+] Нарезка трека $formatted_num ($start_time -> конец)"
                    ffmpeg -hide_banner -loglevel error -y -ss "$start_sec" -i "$flac_name" -map_metadata -1 -c:a flac "$out_track"
                else
                    echo "    [+] Нарезка трека $formatted_num ($start_time -> $bp)"
            ...
            echo "-> Стандартный 16-bit FLAC. Нарезка через shnsplit..."
        ...
        if [ "$split_success" -eq 1 ]; then
            echo "-> Чтение тегов и переименование треков..."
            ...
            if [ "$DELETE_ORIGINAL" -eq 1 ]; then
                echo "-> Безопасная очистка оригиналов и перенос файлов..."
                shopt -s nullglob
            ...
            echo "[✓] Альбом успешно нарезан и обработан!"
        else
            echo "[✗] Ошибка при обработке $flac_name"
            rm -rf "$output_path"
        fi
    )
...
if [ "$DRY_RUN" -eq 1 ]; then
    echo "ИТОГ ПРОВЕРКИ: Найдено $FOUND_COUNT точных пар CUE + FLAC."
fi
